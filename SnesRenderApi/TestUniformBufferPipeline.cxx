...
vk::UniqueDescriptorSetLayout create_uniform_buffer_layot(vk::Device device);
...
TestUniformBufferPipeline::TestUniformBufferPipeline(vk::Device device,
                                                     vk::Format format)
        : render_pass_(create_render_pass(device, format)),
          uniform_buffer_layout_(create_uniform_buffer_layot(device)),
          pipeline_layout_(create_pipeline_layout(device, *uniform_buffer_layout_)),
          graphics_pipeline_(
                  create_graphics_pipeline(device, *pipeline_layout_, *render_pass_)) {}


void TestUniformBufferPipeline::render(RenderTarget& render_target,
                                       const TestUniformBuffers& surface_buffers,
    ...
    vertex_buffer.device_.waitForFences(
            1, &vertex_buffer.copy_fences_[render_target.image_index_], VK_TRUE,
            UINT64_MAX);

    const vma::UniqueBuffer& uniform_buffer =
            surface_buffers.uniform_buffers_[render_target.image_index_];
    float& pangle = *reinterpret_cast<float*>(uniform_buffer.mapMemory());
    pangle = angle;
    ...
    command_buffer.bindVertexBuffers(
            0, 1, &*vertex_buffer.buffers_[render_target.image_index_],
            &vertex_buffer_offset);
    command_buffer.bindDescriptorSets(
            vk::PipelineBindPoint::eGraphics, *pipeline_layout_, 0, 1,
            &surface_buffers.descriptor_sets_[render_target.image_index_], 0, nullptr);

    command_buffer.draw(vertex_buffer.vertex_count_, 1, 0, 0);
...
vk::UniquePipeline create_graphics_pipeline(vk::Device device,
    ...
    vk::UniqueShaderModule vertex_shader(
            shader_module_from_file(device, "shaders/triangle2.spv"));
    vk::UniqueShaderModule fragment_shader(
            shader_module_from_file(device, "shaders/color.spv"));
    ...
    static constexpr vk::VertexInputBindingDescription binding_description =
            vk::VertexInputBindingDescription(0, static_cast<uint32_t>(sizeof(Vertex)));

    static constexpr vk::VertexInputAttributeDescription attribute_descriptions[]{
    ...
    rasterizer_state_info.cullMode = vk::CullModeFlagBits::eFront;
